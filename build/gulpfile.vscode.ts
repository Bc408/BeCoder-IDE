...
function packageTask(platform: string, arch: string, sourceFolderName: string, destinationFolderName: string, _opts?: { stats?: boolean }) {
	...
	const task = () => {
		...
		if (missingNativeBindings.length > 0) {
			throw new Error(`Missing native bindings: ${missingNativeBindings.join(', ')}. Rebuild the listed packages for Electron ${util.getElectronVersion().electronVersion} before packaging.`);
		}
		...
		let result: NodeJS.ReadWriteStream = all
			...
			// BeCoder ships English plus a bundled Simplified Chinese language
			// pack. Electron's other native locale packs are unreachable payload.
			.pipe(util.filter(data => isBeCoderElectronLocale(data.relative, platform)))
			.pipe(filter([
				'**',
				'!LICENSE',
				'!version',
