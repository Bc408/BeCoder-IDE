...
function cleanupDelayMs(): number {
    const seconds = vscode.workspace.getConfiguration('becoder').get<number>('executableCleanupDelaySeconds', DEFAULT_CLEANUP_DELAY_SECONDS);
...
function isCleanupEnabled(): boolean {
	return Configuration.cleanupExecutable();
}
