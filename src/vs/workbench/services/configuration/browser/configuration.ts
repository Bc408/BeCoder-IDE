...
import { Disposable, IDisposable, dispose, MutableDisposable, combinedDisposable, DisposableStore } from '../../../../base/common/lifecycle.js';
...
import { TASKS_CONFIGURATION_KEY, FOLDER_SETTINGS_NAME, IConfigurationCache, ConfigurationKey, FOLDER_SCOPES, WORKSPACE_SCOPES, APPLY_ALL_PROFILES_SETTING, APPLICATION_SCOPES } from '../common/configuration.js';
...
export class FolderConfiguration extends Disposable {
	...
	private createFileServiceBasedConfiguration(fileService: IFileService, uriIdentityService: IUriIdentityService, logService: ILogService) {
		...
		const standAloneConfigurationResources: [string, URI][] = [TASKS_CONFIGURATION_KEY].map(name => ([name, uriIdentityService.extUri.joinPath(this.configurationFolder, `${name}.json`)]));
