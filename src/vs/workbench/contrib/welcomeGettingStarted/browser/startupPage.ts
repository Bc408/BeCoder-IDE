...
import { BeCoderNewTabInput } from '../../becoder/browser/beCoderNewTabInput.js';
...
export class StartupPageRunnerContribution extends Disposable implements IWorkbenchContribution {
	...
	private async run() {
		...
		// BeCoder always keeps an editor surface available: when session
	...
	private async openGettingStarted(showTelemetryNotice?: boolean) {
		const startupEditorTypeID = BeCoderNewTabInput.ID;
		...
		await this.editorService.openEditor(this.instantiationService.createInstance(BeCoderNewTabInput), {
