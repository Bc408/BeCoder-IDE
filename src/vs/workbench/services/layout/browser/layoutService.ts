...
export function shouldShowCustomTitleBar(configurationService: IConfigurationService, window: Window, menuBarToggled?: boolean): boolean {
	...
	// BeCoder uses the editor tab row as its desktop title surface. Do not
	// let a user/profile value for `window.customTitleBarVisibility` restore the
	// removed workbench titlebar.
	if (!isWeb && !hasNativeTitlebar(configurationService)) {
		return false;
	}
