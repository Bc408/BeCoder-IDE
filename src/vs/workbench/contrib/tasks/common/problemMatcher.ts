...
class ProblemMatcherRegistryImpl implements IProblemMatcherRegistry {
	...
	private fillDefaults(): void {
		...
		// GCC/Clang diagnostic matcher kept built in for BeCoder's compiler output.
		this.add({
			name: 'gcc',
			label: localize('gcc', 'GCC compiler problems'),
			owner: 'becoder',
			source: 'gcc',
			applyTo: ApplyToKind.allDocuments,
			fileLocation: FileLocationKind.AutoDetect,
			filePrefix: '${cwd}',
			pattern: {
