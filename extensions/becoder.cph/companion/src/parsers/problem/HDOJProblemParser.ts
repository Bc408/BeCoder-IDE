...
export class HDOJProblemParser extends Parser {
  ...
  public async parse(url: string, html: string): Promise<Sendable> {
    ...
    task.setTimeLimit(parseInt(/Time Limit:\s*\d+\/(\d+)\s*MS/.exec(html)[1], 10));
    task.setMemoryLimit(parseInt(/Memory Limit:\s*\d+\/(\d+)\s*K/.exec(html)[1], 10) / 1024);
    ...
    if (inputs.length !== outputs.length) { throw new Error('Incomplete sample pair.'); }
