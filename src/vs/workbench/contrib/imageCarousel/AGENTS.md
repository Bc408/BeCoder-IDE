...
- **Explorer integration**: The Explorer action builds a collection from selected media files or from the media files in a selected folder.
...
The `workbench.action.openImagesInCarousel` action opens selected media files or the media files in a selected folder. MIME types are resolved via `getMediaMime()` from `src/vs/base/common/mime.ts`.
...
- **Collection identity**: `ImageCarouselEditorInput` uses its collection ID for `EditorInput.matches()` deduplication.
- **Explorer-gated**: `imageCarousel.explorerContextMenu.enabled` controls the Explorer context-menu action.
