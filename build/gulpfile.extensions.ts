...
const compilations = [
	...
	'extensions/becoder.setup/tsconfig.json',
	'extensions/becoder.gcc-diagnostics/tsconfig.json',
	...
	const extensionName = /^extensions\/([^/]+)\//.exec(tsconfigPath)?.[1];
		return !!extensionName && !ext.excludedForOIDistribution.has(extensionName) && !ext.prebuiltOIDistributionExtensions.has(extensionName);
});
