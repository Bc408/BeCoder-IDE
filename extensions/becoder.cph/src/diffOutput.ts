...
export type DiffLine = { lineNumber: number; expected: string | null; received: string | null; type: 'match' | 'changed' | 'missing' | 'extra' };
export type TokenDiff = { token: string; status: 'match' | 'extra' | 'missing' };
export type DiffResult = { isMatch: boolean; lines: DiffLine[]; summary: string; tokenDiff: TokenDiff[] };
...
export function diffOutput(expected: string, received: string): DiffResult {
    ...
    // Bound the upstream quadratic table for large program output.
    const tokenDiff = expTokens.length * recTokens.length <= 1000000 && expTokens.length + recTokens.length <= 20000
        ? lcsTokenDiff(expTokens, recTokens)
        : [{ token: expected.slice(0, 50000), status: 'missing' as const }, { token: received.slice(0, 50000), status: 'extra' as const }];
