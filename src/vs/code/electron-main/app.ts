...
`;
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// OI: agent host disabled. @github/copilot-sdk is filtered out of the
		// packaged node_modules (see build/gulpfile.vscode.ts packageTask), so
		// instantiating the starter/manager would spawn agentHostMain and crash
		// with ERR_MODULE_NOT_FOUND on import of '@github/copilot-sdk'.
