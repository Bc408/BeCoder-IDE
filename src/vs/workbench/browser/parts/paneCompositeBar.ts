...
export class PaneCompositeBar extends Disposable {
	...
	private static readonly beCoderActivityBarMigrationKey = 'workbench.activity.becoderMinimalDefaultsApplied.v4';
	...
	private get isBeCoderActivityBar(): boolean {
	...
	constructor(
		...
		this.applyBeCoderActivityBarDefaults();
	...
	private applyBeCoderActivityBarDefaults(): void {
		if (!this.isBeCoderActivityBar || this.storageService.getBoolean(PaneCompositeBar.beCoderActivityBarMigrationKey, StorageScope.PROFILE, false)) {
			return;
		}
		for (const { id } of this.cachedViewContainers) {
			if (PaneCompositeBar.beCoderDefaultActivityContainers.has(id)) {
				this.compositeBar.pin(id);
			} else {
				this.compositeBar.unpin(id);
			}
		}
		this.storageService.store(PaneCompositeBar.beCoderActivityBarMigrationKey, true, StorageScope.PROFILE, StorageTarget.USER);
	}
