...
configurationRegistry.registerConfigurations(product.updateUrl ? [{
	id: 'update',
	order: 15,
	...
	properties: {
		'update.mode': {
			...
			description: localize('updateMode', "Configure whether BeCoder receives automatic application updates. Updates are fetched from the BeCoder update service."),
			...
			enumDescriptions: [
				...
				localize('default', "Enable automatic update checks. BeCoder will check for updates automatically and periodically.")
			],
			policy: {
				name: 'UpdateMode',
				...
				minimumVersion: '1.67',
				localization: {
					description: { key: 'updateMode', value: localize('updateMode', "Configure whether BeCoder receives automatic application updates. Updates are fetched from the BeCoder update service."), },
					enumDescriptions: [
						{
							key: 'none',
						...
						{
							key: 'default',
							value: localize('default', "Enable automatic update checks. BeCoder will check for updates automatically and periodically."),
						}
					]
				},
			}
		...
		'update.enableWindowsBackgroundUpdates': {
			...
			description: localize('enableWindowsBackgroundUpdates', "Enable to download and install new BeCoder versions in the background."),
		...
		'update.showReleaseNotes': {
			...
			default: true,
			scope: ConfigurationScope.APPLICATION,
			description: localize('showReleaseNotes', "Show Release Notes after an update. The Release Notes are fetched from the BeCoder update service."),
			tags: ['usesOnlineServices'],
		},
