...
export const excludedForOIDistribution = new Set([
	...
	'handlebars', 'hlsl', 'html', 'html-language-features', 'ini', 'jake', 'java', 'javascript',
	'jeff-hykin.better-cpp-syntax', 'julia', 'less', 'lua', 'media-preview', 'merge-conflict', 'microsoft-authentication', 'node_modules', 'npm',
	...
	'terminal-suggest', 'typescript-basics', 'typescript-language-features', 'vb', 'vscode-api-tests',
...
function doPackageLocalExtensionsStream(forWeb: boolean, disableMangle: boolean, native: boolean): Stream {
	...
	const localExtensionsDescriptions = (
		(glob.sync('extensions/*/package.json') as string[])
			...
			.filter(({ name }) => !excludedForOIDistribution.has(name))
			.filter(({ name }) => builtInExtensions.every(b => b.name !== name))
			.filter(({ manifestPath }) => (forWeb ? isWebExtension(require(manifestPath)) : true))
	);

	// `vsce.listFiles` respects each extension's .vscodeignore.  That is normally
	// desirable, but it can omit package.nls*.json files we add while localizing
	// bundled third-party extensions.  A manifest containing %key% without its
	// package.nls.json is displayed literally, so always include those bundles.
	const packageNlsPatterns = localExtensionsDescriptions.map(extension => `${path.relative(root, extension.path)}/package.nls*.json`);
	const packageNlsStream = packageNlsPatterns.length > 0
		? gulp.src(packageNlsPatterns, { base: root, allowEmpty: true })
		: es.readArray([]);
...
].filter(({ script }) => !excludedForOIDistribution.has(script.split('/')[0]));
